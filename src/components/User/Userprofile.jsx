...
const UserProfile = () => {
  ...
  if (!profileData) {
    return <div className="text-center text-white mt-20">Error loading profile data.</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-mono pt-8 pb-20 px-4 md:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="relative w-full h-48 md:h-64 bg-gray-900 rounded-xl overflow-hidden border border-gray-800 shadow-2xl">
          {profileData.banner_image ? (
            <img src={profileData.banner_image} alt="Banner" className="w-full h-full object-cover opacity-60" />
          ) : (
          ...
          <div className="absolute bottom-6 left-6 md:bottom-8 md:left-8 flex items-end gap-6">
            <div className="w-24 h-24 md:w-32 md:h-32 rounded-xl bg-[#0a0a0c] border-2 border-cyan-500 flex items-center justify-center shadow-[0_0_20px_rgba(8,145,178,0.4)]">
              <span className="text-5xl md:text-7xl font-black text-cyan-500 uppercase">
            ...
            <div className="mb-2">
              ...
              <p className="text-cyan-400 text-sm md:text-base font-bold uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-[#0a0a0c] border border-gray-800 rounded-xl p-6 relative overflow-hidden group hover:border-cyan-500/50 transition-colors duration-300">
              ...
              <div className="flex gap-3">
                <button className="flex-1 bg-white text-black py-2 rounded text-xs font-black uppercase tracking-widest hover:bg-gray-200 transition-colors">
                  Deposit
                </button>
                <button className="flex-1 bg-transparent border border-gray-600 text-white py-2 rounded text-xs font-black uppercase tracking-widest hover:border-white transition-colors">
                  Withdraw
                </button>
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-[#0a0a0c] border border-gray-800 rounded-xl p-6 h-full">
              <div className="flex justify-between items-center border-b border-gray-800/50 pb-4 mb-4">
                ...
                <span className="text-xs text-cyan-500 font-bold uppercase tracking-widest cursor-pointer hover:text-cyan-400">View All</span>
              </div>

              {profileData.recent_transactions && profileData.recent_transactions.length > 0 ? (
                <div className="space-y-4">
                  {profileData.recent_transactions.map((tx) => (
                    <div key={tx.id} className="flex justify-between items-center p-4 bg-black/40 border border-gray-800/50 rounded hover:border-gray-700 transition-colors">
                      <div>
                      ...
                      <div className={`text-lg font-black ${['DEPOSIT', 'PRIZE', 'REFUND'].includes(tx.transaction_type) ? 'text-green-400' : 'text-red-400'}`}>
                        {['DEPOSIT', 'PRIZE', 'REFUND'].includes(tx.transaction_type) ? '+' : '-'}₹{tx.amount}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12 opacity-50">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-400">No recent transactions</p>
                </div>
              )}
            </div>
          </div>
