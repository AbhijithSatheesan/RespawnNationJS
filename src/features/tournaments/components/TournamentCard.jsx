...
const TournamentCard = ({ tournament }) => {
  ...
  // State to control the Razorpay Modal
  ...
  const handleActionClick = (e) => {
    // Prevent the click from bubbling up if you wrap the whole card in a link later
    e.stopPropagation(); 
    
    // If it's open, show the payment modal! Otherwise, go to standings.
  ...
  const handlePaymentSuccess = () => {
    ...
    // Optional: Refresh the page or trigger a re-fetch so the slots update to show +1 player
      ...
      <div className="mt-auto border-t border-gray-700/60 pt-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <button 
          ...
          className={`w-full md:w-32 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all border shadow-lg backdrop-blur-sm z-20 relative ${
            ...
            // 1. If Open AND Already Registered
            tournament.is_registered && tournament.status === 'REGISTRATION' 
              ? 'bg-gray-800 text-green-500 border-green-500/30 cursor-not-allowed shadow-[0_0_10px_rgba(34,197,94,0.2)]' :
            
            // 2. If Open AND NOT Registered
            tournament.status === 'REGISTRATION' 
              ? 'bg-cyan-600 border-cyan-500 text-white hover:bg-cyan-500 hover:shadow-[0_0_15px_rgba(8,145,178,0.6)] cursor-pointer' :
            
            // 3. Generating Brackets
            tournament.status === 'GENERATING' 
              ? 'bg-black/50 text-yellow-500 border-yellow-500/50 hover:bg-yellow-900/40 cursor-pointer' :
            
            // 4. Live
            tournament.status === 'LIVE' 
              ? 'bg-black/50 text-red-500 border-red-500/50 hover:bg-red-900/40 cursor-pointer' :
            
            // 5. Completed
            'bg-gray-800/80 text-gray-300 border-gray-600 hover:bg-gray-700 hover:text-white cursor-pointer'
        }`}>
  ...
  return (
    <>
      {/* We conditionally render Layout A (Banner) or Layout B (Grid) based on the data. 
        Both use the same TournamentDetails component to keep code DRY.
      */}
      ...
      {/* Render the Payment Modal if isModalOpen is true */}
      {isModalOpen && (
        <JoinTournamentModal 
